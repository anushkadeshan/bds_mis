...
use App\Models\BD_CSOparticipants;
use App\Models\BdPhotos;
use App\Models\BusinessDevelopment;


class BdCreate extends Component
{
    use WithFileUploads;
    ...
    public function saveData(){
        $this->validate();
        $attendance_name  = $this->attendance->store('bd_attendance');
        //dd($attendance_name);
        $data = BusinessDevelopment::create([
        ...
        foreach ($this->photos as $photo) {
            $photo_name = $photo->store('bd_photos');
            BdPhotos::create([
                'name' => $photo_name,
                'bd_id' => $lastId
            ]);
...
//$this->clearForm();
    ...
    {
        return view('livewire.bd-create');
    }
}

...
use App\Models\ML_Photos;
use Livewire\WithFileUploads;
use App\Models\ResoursePerson;
use App\Models\LivelihoodFamily;
use App\Models\MarketingLinkage;
use App\Models\ML_CSOparticipant;
use Illuminate\Support\Facades\Auth;

class CreateMarketingLinkages extends Component
{
    use WithFileUploads;
    ...
    public function saveData(){
        $this->validate();
        $attendance_name  = $this->attendance->store('ml_attendance');
        //dd($attendance_name);
        $data = MarketingLinkage::create([
        ...
        foreach ($this->photos as $photo) {
            $photo_name = $photo->store('ml_photos');
            ML_Photos::create([
                'name' => $photo_name,
                'ml_id' => $lastId
            ]);
...
//$this->clearForm();
    ...
    {
        return view('livewire.create-marketing-linkages');
    }
}

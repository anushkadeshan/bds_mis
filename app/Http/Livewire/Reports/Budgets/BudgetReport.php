...
class BudgetReport extends Component
{
    ...
    public function queryBudget(){
        ...
        if($user->hasRole(['Community Development coordinator', 'Youth Development coordinator'])){
            ...
            $this->gnds = Budget::with('gnd')->distinct('gn_id')->where('added_by',$user->id)->whereNotNull('gn_id')->get()->pluck('gnd.name','gn_id')->toArray();
            $query = $query->with('precondition','dsd','gnd','activity','addedBy')->where('added_by',$user->id)->where('approved',true)->orderBy('pre_condition_id');
        ...
        elseif($user->hasRole(['Regional Manager', 'M&E Staff', 'District Represetnative'])){
            ...
            $this->gnds = Budget::with('gnd')->distinct('gn_id')->whereIn('added_by',json_decode($subordinates))->whereNotNull('gn_id')->get()->pluck('gnd.name','gn_id')->toArray();
            $this->staffs = User::whereIn('added_by',json_decode($subordinates))->pluck('name','id');
            $this->budgets = Budget::with('precondition','dsd','gnd','activity')->whereIn('added_by',json_decode($subordinates))->
            $query = $query->with('precondition','dsd','gnd','activity','addedBy')->whereIn('added_by',json_decode($subordinates))->where('approved',true)->orderBy('pre_condition_id');
            if(empty(!$this->selectedDistrict)){
                $query = $query->whereIn('district',$this->selectedDistrict);
            }
            if(empty(!$this->selectedDsd)){
            ...
            $this->gnds = Budget::with('gnd')->distinct('gn_id')->whereNotNull('gn_id')->get()->pluck('gnd.name','gn_id')->toArray();
            $this->regional_managers = User::role('Regional Manager')->pluck('name','id');
            $this->staffs = User::pluck('name','id');

            $query = $query->with('precondition','dsd','gnd','activity','addedBy')->where('approved',true)->orderBy('pre_condition_id');
            if(empty(!$this->selectedDistrict)){
                $query = $query->whereIn('district',$this->selectedDistrict);
            }
            if(empty(!$this->selectedDsd)){

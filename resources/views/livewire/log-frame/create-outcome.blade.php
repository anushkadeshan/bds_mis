    ...
    <div class="modal-dialog modal-lg">
            ...
            <div class="modal-body">
                ...
                <div class="flex">
                    ...
                    <input wire:model="outcome" class="border border-2 rounded-r px-4 py-2 w-full" type="text" name="name" spellcheck="false" data-ms-editor="true">

                </div>
                @error('outcome') <span class="text-danger">*{{ $message }}</span> @enderror
                <br/>
                <div class="flex">
                    <span class="text-sm border border-2 w-25 rounded-l px-4 py-2 bg-gray-300 whitespace-no-wrap">Pre Condition :</span>
                    <select wire:model="pre_condition" class="border border-2 rounded-r px-4 py-2 w-full" >
                        <option value="">Select Option</option>
                        @foreach($pre_conditions as $p)
                                    <option value="{{$p->id}}" >{{$p->pre_condition}}</option>
                                @endforeach
                            </select> @error('pre_condition') <span class="text-danger">*{{ $message }}</span> @enderror
                </div>
            </div>

...
<div x-cloak x-data="{ open: false }" x-on:keydown.escape.stop="open = false" x-on:mousedown.away="open = false"
    ...
    <ul style="width: 280px;" class="dropdown-menu w-200" x-bind:class="{'show' : open}" role="menu">
        <li>
            @if ($filtersView)
            @include($filtersView)
            ...
            <button wire:click.prevent="resetFilters" x-on:click="open = false" class="text-center dropdown-item">
                @lang('Clear')
            </button>
            @endif
        </li>
